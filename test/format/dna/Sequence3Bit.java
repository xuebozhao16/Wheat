...
public class Sequence3Bit implements SequenceInterface {
    ...
    private void initialize (String seq) {
        ...
        byte[] seqByte = seq.toUpperCase().getBytes(); //toUpperCase() 方法用于把字符串转换为大写。//getBytes(): 使用平台的默认字符集将字符串编码为 byte 序列，并将结果存储到一个新的 byte 数组中。
    ...
    /**
     * Set the base in bit set from a position index of DNA sequence //想要对fasta文件处理的位置信息
     * @param positionIndex index of DNA sequence   这里就是位操作了
    ...
    protected void setCodedBase (int positionIndex, byte value) {
        ...
        for (int i = 0; i < wordSize; i++) {
            boolean b = (v & 1) == 1;   //如果相对应位都是1，则结果为1，否则为0 eg.  A = 00111100 B = 00001101 A&b = 00001100
            v = (byte) (v >> 1);        //按位右移运算符。左操作数按位右移右操作数指定的位数。A >> 2得到15即 1111
            seqS.set(startIndex+i, b);  //这里的b是1或是0，若b是1，指的碱基是C
        }
    }
    ...
    private static HashByteByteMap buildByteAscIIMap () { //这是和上一个不一样的的方法，使用的是Byte
    ...
    @Override
    public double getProportionA() { //得到部分A
        int cnt = 0;
